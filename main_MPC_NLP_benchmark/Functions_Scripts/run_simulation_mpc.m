...
%     [par.G_1r, par.G_2r, par.G_3r] = get_poly(par.G_nlp, 0, 0+60*60*4);
%     [par.fW_1r, par.fW_2r, par.fW_3r] = get_poly(par.fW_nlp, 0, 0+60*60*4);
%     [par.sW_1r, par.sW_2r, par.sW_3r] = get_poly(par.sW_nlp, 0, 0+60*60*4);
%     [par.temp_1r, par.temp_2r, par.temp_3r] = get_poly(par.temp_nlp, 0, 0+60*60*4);
...
%         if x0(1) > par.Route.max_v(par.iter_initial+par.iter_mpc+1)
%             x0(1) = par.Route.max_v(par.iter_initial+par.iter_mpc+1);
%         end
    
        % update G polynomial fit
%         [par.G_1, par.G_2, par.G_3] = get_poly(par.G_mpc, x0(3), x0(3) + par.N_t);
%         [par.fW_1, par.fW_2, par.fW_3] = get_poly(par.fW_mpc, x0(3), x0(3) + par.N_t);
%         [par.sW_1, par.sW_2, par.sW_3] = get_poly(par.sW_mpc, x0(3), x0(3) + par.N_t);
%         [par.temp_1, par.temp_2, par.temp_3] = get_poly(par.temp_mpc, x0(3), x0(3) + par.N_t);
    ...
    par.final_velocity_mpc = OptRes.xx(1,end);
    par.final_E_bat_mpc = OptRes.xx(2,end);
