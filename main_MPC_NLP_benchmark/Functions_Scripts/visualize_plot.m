...
figure
subplot(3,1,1)
plot(OptResNLP.xx1(:,1)*3.6), hold on
plot(OptRes.xx(1,:)*3.6), hold on
...
plot(OptResNLP.xx1(:,2)/par.E_bat_max), hold on
plot(OptRes.xx(2,:)/par.E_bat_max), hold on
plot(par.E_bat_target_DP(1+par.iter_initial:par.iter_initial+size(OptRes.xx,2))/par.E_bat_max)
...
plot(OptResNLP.u_cl(:,1),'linewidth',1.), hold on
plot(OptResNLP.u_cl(:,2),'linewidth',0.1,'color','r')

plot(OptRes.u_cl(:,1),'linewidth',1.)
plot(OptResNLP.u_cl(:,2),'linewidth',0.1,'color','g')

title('Control Input')
legend('nlp', 'nlp (brake)', 'mpc', 'mpc (brake)')
xlim([0 par.s_tot/par.s_step])
xlabel('[km]')
ylabel('[W]')
