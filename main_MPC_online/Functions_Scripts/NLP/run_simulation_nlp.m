...
%% NLP optimization over entire horizon
    ...
    par.N_t = par.N*par.s_step/8.333; % prediction in time domain, how far the weather is interpolated, assuming minimum velocity is 30 km/h (8.33 m/s)
    ...
    [par.G_1, par.G_2, par.G_3] = get_poly(weather.G_data, 0, par.N_t);
    [par.fW_1, par.fW_2, par.fW_3] = get_poly(weather.fW_data, 0, par.N_t);
    [par.sW_1, par.sW_2, par.sW_3] = get_poly(weather.sW_data, 0, par.N_t);
    [par.temp_1, par.temp_2, par.temp_3] = get_poly(weather.temp_data, 0, par.N_t);
