...
function [par, OptRes] = run_simulation_mpc_online(par, weather, args, f, solver, s_0, t_0)
    % Start MPC
    ...
    [p_G, p_fW, p_rho, p_temp] = get_polynomial_weather_fit(weather, s_0, t_0, par.N_t);
    [p_G_r, p_fW_r, p_rho_r, p_temp_r] = get_polynomial_weather_fit(weather, s_0, t_0, 0);
    ...
    while par.iter_mpc < par.iter_mpc_max
        ...
        OptRes.xdist(par.iter_mpc+2) = s_0+par.s_step;
        ...
        % pause the simulation so that the simulation is going side by side
        % with the reality!
        if iter_time_1+iter_time_2-0.05 < par.s_step/x0(1)
            pause(par.s_step/x0(1)-iter_time_1-iter_time_2-0.05)
        else
        ...
        %%
        [p_G, p_fW, p_rho, p_temp] = get_polynomial_weather_fit(weather, s_0, x0(3), par.N_t);
        [p_G_r, p_fW_r, p_rho_r, p_temp_r] = get_polynomial_weather_fit(weather, s_0, x0(3), 0);
...
%     printresult.feasible = diagnostics.alwaysFeasible;              % if = 1 always feasible, 0 otherwise
