function plotOptTrajectory(OptRes, par, s, x0)
    ...
    ylim([40 140]);
    legend('car', 'predicted', 'max', 'Location','northeastoutside');
    xlabel('[km]'); % Label x-axis
    ylabel('[km/h]'); % Label y-axis
    mean_vel = mean(OptRes.trajectory(2:end,1)*3.6);
    yline(mean_vel,'-', ['mean = ', num2str(mean_vel), 'km/h'] ,'Color','g','DisplayName','mean vel');
    ...
    legend('car', 'predicted', 'target', 'Location','northeastoutside');
    ...
    yline(mean_input,'-', ['mean = ', num2str(mean_input), 'W'] ,'Color','g');
