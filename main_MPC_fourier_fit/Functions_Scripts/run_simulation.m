...
function [par, OptRes] = run_simulation(par, args, f, solver, s_0, t_0)
    ...
    par.G_var = get_G(par.n_waves,par.G_MPC, 0, 0+ par.N*par.s_step/16.6);

    % initialize polynomial fit of G used for real conditions shift
    par.G_var_shift = get_G(par.n_waves,par.G_real, 0, 0+ par.N*par.s_step/16.6);
    ...
    while par.iter_mpc < par.iter_mpc_max
        ...
        par.G_var = get_G(par.n_waves,par.G_MPC, x0(3), x0(3) + par.N*par.s_step/16.6);

        % update real G polynomial
        par.G_var_shift = get_G(par.n_waves,par.G_real, x0(3), x0(3) + par.N*par.s_step/16.6);
