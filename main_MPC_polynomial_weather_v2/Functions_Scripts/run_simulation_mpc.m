...
function [par, OptRes] = run_simulation_mpc(par, args, f, solver, s_0, t_0)
    ...
    [par.G_1, par.G_2, par.G_3] = get_poly(par.G_data, 0, 60*15*4*4);
    [par.fW_1, par.fW_2, par.fW_3] = get_poly(par.fW_data, 0, 60*15*4*4);
    [par.sW_1, par.sW_2, par.sW_3] = get_poly(par.sW_data, 0, 60*15*4*4);
    [par.temp_1, par.temp_2, par.temp_3] = get_poly(par.temp_data, 0, 60*15*4*4);
    ...
    while par.iter_mpc < par.iter_mpc_max
        ...
        OptRes.xG(par.iter_mpc+1) = par.G_1r*(x0(3)/60/15)^2 + par.G_2r*(x0(3)/60/15) + par.G_3r;
        OptRes.xwind_front(par.iter_mpc+1) = par.fW_1r*(x0(3)/60/15)^2 + par.fW_2r*(x0(3)/60/15) + par.fW_3r;
        OptRes.xwind_side(par.iter_mpc+1) = par.sW_1r*(x0(3)/60/15)^2 + par.sW_2r*(x0(3)/60/15) + par.sW_3r;
        OptRes.xtheta(par.iter_mpc+1) = par.temp_1r*(x0(3)/60/15)^2 + par.temp_2r*(x0(3)/60/15) + par.temp_3r;
...
%                                                             par.temp_3]);
        ...
        % update G polynomial fit of real shift
