...
OPT_variables = [reshape(X,n_states*(N+1),1);reshape(U,n_controls*N,1); S];           % OPT_variables = [x1, x2, x3, ... xN, u1, u2, u3, ..., uN-1]
...
xS(1) = S0;                      % xS contains the history of slack
dist(1) = h0;
...
args.lbx(1:n_states:n_states*(N+1),1) = 55/3.6; %par.Route.min_v(iter_initial+iter_mpc+1:iter_initial+N+1+iter_mpc); %state velocity lower bound
...
while iter_mpc < iter_max+1
    ...
    %vars_update_pred
    % initial value of the optimization variables, warm start!
    args.x0  = [reshape(X0',n_states*(N+1),1);reshape(u0',n_controls*N,1); S0]; 
    ...
    u = reshape(full(sol.x(n_states*(N+1)+1:end-1))',n_controls,N)'; % get controls only from the solution
    u_cl= [u_cl ; u(1,:)];
