...
function [par, g_nlp, args] = initialize_constraints(par, f, X, U, P, S)
    ...
    for k = 1:par.N     
        ...
        w = [P(par.n_states+ k);                               % inclination    
             P(par.n_states+par.N + k);                        % irradiation
             P(par.n_states+par.N+par.N + k);                  % front wind
             P(par.n_states+par.N+par.N+par.N + k);            % side wind
             P(par.n_states+par.N+par.N+par.N+par.N + k)];     % temperature
    ...
    g.SoC_target = [X(2,par.N+1) - P(par.n_states+par.n_vars*par.N+1) + S];     
    ...
    args.ubx(1:par.n_states:par.n_states*(par.N+1),1) = 32;                     
