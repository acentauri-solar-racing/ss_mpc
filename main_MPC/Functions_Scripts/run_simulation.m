...
function [par, OptRes] = run_simulation(par, args, f, solver, s_0, t_0)
    ...
    vars_update_pred = [simvar.alpha(par.iter_initial+par.iter_mpc+1:par.iter_initial+par.iter_mpc+1+(par.N-1)); 
                   par.G_int(linspace(t_0, t_0+ par.s_step*par.N/v_0,par.N))';                              
                   diag(par.wind_front_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),linspace(t_0, t_0+ par.s_step*par.N/v_0,par.N)}));
                   diag(par.wind_side_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),linspace(t_0, t_0+ par.s_step*par.N/v_0,par.N)})); 
                   diag(par.theta_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),linspace(t_0, t_0+ par.s_step*par.N/v_0,par.N)}))];
    ...
    while par.iter_mpc < par.iter_mpc_max+1
        ...
        vars_update_pred = [simvar.alpha(par.iter_initial+par.iter_mpc+1 +1:par.iter_initial+par.iter_mpc+1+(par.N-1) +1); 
                           par.G_int(OptRes.xx1(2:end,3,par.iter_mpc+1)/60);                              
                           diag(par.wind_front_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),OptRes.xx1(2:end,3,par.iter_mpc+1)/60/60}));
                           diag(par.wind_side_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),OptRes.xx1(2:end,3,par.iter_mpc+1)/60/60})); 
                           diag(par.theta_int({linspace(s_0,s_0+(par.N-1)*par.s_step,par.N),OptRes.xx1(2:end,3,par.iter_mpc+1)/60/60}))];     
