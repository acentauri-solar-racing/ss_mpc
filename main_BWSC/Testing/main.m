...
% input constraints
g.u_rate = [];
g.u_rate = [g.u_rate; P(n_states+n_vars*N+1) - U(1,1)] ;   %U(0) - U(1)     % P = [x_0(n_states);grade(N);inclination(N);var.v_front(N);var.v_side(N); amb_temp(N); U(k-1)(1); SoC_target_lastN(1);X(2,k-1)(1)]
for k = 1:N-1
...
g.SoC_lb = [];
% for k = 1:N
    g.SoC_lb = [g.SoC_lb; X(2,N+1) - P(n_states+n_vars*N+1+ 1)];     %SoC - SoC_lowerbound_predicted
% end
...
% g.acc = [];
% g.acc = [g.acc; (X(2,1) - P(n_states+n_vars*N+1+1 + 1))/(X(3,1) - P(n_states+n_vars*N+1+1+1 +1)+ 1e-6)] ;   %V(1) - V(0)     % P = [x_0(n_states);grade(N);inclination(N);var.v_front(N);var.v_side(N); amb_temp(N); U(k-1)(1); SoC_target_lastN(1);X(2,k-1)(1);X(3,k-1)(1)]
% for k = 1:N-1
%     g.acc = [g.acc; (X(2,k+1) - X(2,k))/(X(3,k+1) - X(3,k)+ 1e-6)];
% end
...
args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -inf; % P_brake lower bound
...
vars_update = [simvar.alpha(iter_initial+1+iter_mpc:iter_initial+1+iter_mpc+(N-1)); 
...
args.ubx(1:n_states:n_states*(N+1),1) = par.Route.max_v(iter_initial+1+iter_mpc:iter_initial+N+1+iter_mpc); %state velocity upper bound
...
main_loop = tic;
iter_max = sim_dist/h;
while iter_mpc < iter_max

    args.p = [x0; vars_update; u_prev(1); SoC_target(iter_initial+N+iter_mpc); v_prev; t_prev]; % set the values of the parameters vector, set the slope prediction = [x0; road_slope_over_horizon N]
    ...
    xG(iter_mpc+1) =  par.G_int(xx1(1,3,iter_mpc+1)/60);
    xwind_front(iter_mpc+1) = par.wind_front_int({h0,xx1(2,3,iter_mpc+1)/60/60});
    xwind_side(iter_mpc+1) = par.wind_side_int({h0,xx1(2,3,iter_mpc+1)/60/60});
    xtheta(iter_mpc+1) = par.theta_int({h0,xx1(2,3,iter_mpc+1)/60/60});

    % Apply the control and shift the solution
    % IN REAL LIFE ONLINE APPLICATION, INSTEAD
%     [h0, x0, u0] = shift(h, h0, x0, u, f, [simvar.alpha(iter_initial+1+iter_mpc); ...
%                                            simvar.G(iter_initial+1+iter_mpc); ...
%                                            simvar.v_front(iter_initial+1+iter_mpc); ...
%                                            simvar.v_side(iter_initial+1+iter_mpc); ...
%                                            simvar.theta_amb(iter_initial+1+iter_mpc)]);      %update the states, 

    [h0, x0, u0] = shift(h, h0, x0, u, f, [simvar.alpha(iter_initial+1+iter_mpc); ...          % x_0 -> x_1
                                       par.G_int(xx1(1,3,iter_mpc+1)/60);
                                       par.wind_front_int({h0,xx1(1,3,iter_mpc+1)/60/60}); ...
                                       par.wind_side_int({h0,xx1(1,3,iter_mpc+1)/60/60}); ...
                                       par.theta_int({h0,xx1(1,3,iter_mpc+1)/60/60})]);

      % update initial state, update meteo condition prediction
%     vars_update = [simvar.alpha(iter_initial+1+iter_mpc:iter_initial+N+iter_mpc); 
%                    simvar.G(iter_initial+1+iter_mpc:iter_initial+N+iter_mpc);
%                    simvar.v_front(iter_initial+1+iter_mpc:iter_initial+N+iter_mpc);
%                    simvar.v_side(iter_initial+1+iter_mpc:iter_initial+N+iter_mpc); 
%                    simvar.theta_amb(iter_initial+1+iter_mpc:iter_initial+N+iter_mpc)];
...
%     x0(1) = x0(1) + rand/3.6 - rand/3.6;                 %disturbance of 1 km/h
%     x0(1) = x0(1) - rand*par.E_bat_max*0.00001 + rand*par.E_bat_max*0.00001; % disturbance of 0.01% SoC
