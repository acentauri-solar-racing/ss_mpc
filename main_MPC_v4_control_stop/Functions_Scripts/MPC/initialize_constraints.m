...
function [par, g_nlp, args] = initialize_constraints(par, f, X, U, P, S1, S2)
    ...
    g.CS = []
    % control stop constraint
    % v(k) - (s_CS - s(k+N))
    % if s(k+N) = s_CS
    % v(k) - (s_CS - s(k+N))
    for k = 1:par.N+1
        g.CS = [g.CS; X(1,k) - (P(par.n_states + par.N + 12 +1)-P(par.n_states + par.N + 12 +1 +k))];
    end

    % compose final constraints vector
    g_nlp = [g_nlp; g.x0; g.st_next; g.SoC_target; g.max_v; g.CS];
    ...
    args.lbg(par.n_states*(par.N+1)+1+par.N+1 +1:par.n_states*(par.N+1)+1+par.N+2+par.N) = -inf;                                                 
    args.ubg(par.n_states*(par.N+1)+1+par.N+1 +1:par.n_states*(par.N+1)+1+par.N+2+par.N) = inf;  
